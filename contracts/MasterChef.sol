...
contract MasterChef is Ownable, ReentrancyGuard {
	...
	// Bee referral contract address.
	IBeeReferral public beeReferral;
	...
	function pendingBee(uint256 _pid, address _user) external view returns (uint256) {
		...
		uint256 pending = user.amount.mul(accBeePerShare).div(1e12).sub(user.rewardDebt);
		return pending.add(user.rewardLockedUp);
	}

	// View function to see if user can harvest BEEs.
	...
	function updatePool(uint256 _pid) public {
		...
		uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
		uint256 beeReward = multiplier.mul(beePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
		bee.mint(devAddress, beeReward.div(10));
	...
	function deposit(
		...
		updatePool(_pid);
		if (_amount > 0 && address(beeReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
			beeReferral.recordReferral(msg.sender, _referrer);
		}
	...
	// Pay or lockup pending BEEs.
	function payOrLockupPendingBee(uint256 _pid) internal {
		PoolInfo storage pool = poolInfo[_pid];
		UserInfo storage user = userInfo[_pid][msg.sender];
		...
		if (canHarvest(_pid, msg.sender)) {
			if (pending > 0 || user.rewardLockedUp > 0) {
				...
				// send rewards
				safeBeeTransfer(msg.sender, totalRewards);
	...
	function updateEmissionRate(uint256 _beePerBlock) public onlyOwner {
		massUpdatePools();
		emit EmissionRateUpdated(msg.sender, beePerBlock, _beePerBlock);
		beePerBlock = _beePerBlock;
	}

	// Update the bee referral contract address by the owner
	function setBeeReferral(IBeeReferral _beeReferral) public onlyOwner {
		beeReferral = _beeReferral;
	}
	...
	function payReferralCommission(address _user, uint256 _pending) internal {
		if (address(beeReferral) != address(0) && referralCommissionRate > 0) {
			address referrer = beeReferral.getReferrer(_user);
			uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

			if (referrer != address(0) && commissionAmount > 0) {
				bee.mint(referrer, commissionAmount);
				beeReferral.recordReferralCommission(referrer, commissionAmount);
