...
// BeeToken with Governance.
contract BeeToken is BEP20 {
	...
	uint256 public antiwhaleLiquidityRate = 2000;
	...
	modifier antiWhale(
		...
		if (maxTransferAmount() > 0) {
			if (_excludedFromAntiWhale[sender] == false && _excludedFromAntiWhale[recipient] == false && amount >= maxTransferAmount()) {
				uint256 liquidityAmount = amount.mul(antiwhaleLiquidityRate).div(10000);
	...
	/**
	 * @notice Constructs the BeeToken contract.
	 */
	constructor() public BEP20("Bee Token", "BEE") {
	...
	/// @dev overrides transfer function to meet tokenomics of BEE
	function _transfer(
		address sender,
		address recipient,
		uint256 amount
		...
		if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
			...
			require(taxAmount == burnAmount + liquidityAmount, "BEE::transfer: Burn value invalid");

			// default 95% of transfer sent to recipient
			uint256 sendAmount = amount.sub(taxAmount);
			require(amount == sendAmount + taxAmount, "BEE::transfer: Tax value invalid");
	...
	function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
		require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "BEE::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
	...
	function updateBurnRate(uint16 _burnRate) public onlyOperator {
		require(_burnRate <= 100, "BEE::updateBurnRate: Burn rate must not exceed the maximum rate.");
	...
	function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
		require(_maxTransferAmountRate <= 10000, "BEE::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
	...
	function updateAntiwhaleLiquidityRate(uint256 _antiwhaleLiquidityRate) public onlyOperator {
		antiwhaleLiquidityRate = _antiwhaleLiquidityRate;
	}
	...
	function updateSwapRouter(address _router) public onlyOperator {
		...
		require(swapPair != address(0), "BEE::updateSwapRouter: Invalid pair address.");
	...
	function transferOperator(address newOperator) public onlyOperator {
		require(newOperator != address(0), "BEE::transferOperator: new operator is the zero address");
